class NumberToName

  def self.numbers_name
    @numbers_name = {
      1000 => "one thousand", 100 => "one hundred",
      90 => "ninety",   80 => "eighty",   70 => "seventy",
      60 => "sixty",    50 => "fifty",    40 => "forty",
      30 => "thirty",   20 => "twenty",   19 => "nineteen",
      18 => "eighteen", 17 => "seventeen",16 => "sixteen",
      15 => "fifteen",  14 => "fourteen", 13 => "thirteen",
      12 => "twelve",   11 => "eleven",   10 => "ten",
      9  => "nine",     8  => "eight",    7  => "seven",
      6  => "six",      5  => "five",     4  => "four",
      3  => "three",    2  => "two",      1  => "one",
      0  => "zero"
    }
  end

  def self.numbers_to_words(number)

    if numbers_name.has_key?(number)
      numbers_name[number]
    elsif number < 0
      return "enter a positive number"
    elsif number.to_s.length == 2
      tens = numbers_name[number - (number % 10)]
      singles = numbers_name[number % 10]
      tens + " " + singles
    elsif number.to_s.length == 3
      hundred = numbers_name[number / 100] + " hundred"
      tens = numbers_name[number - ((number / 100) * 100) - (number % 10)]
      singles = numbers_name[number % 10]
      if tens == "zero"
        tens = "and"
      end
      hundred + " " + tens + " " + singles
    elsif number.to_s.length == 4
      get_thousand = number/1000
      left = number - (get_thousand*1000)
      thousand = numbers_name[get_thousand] + " thousand"
      hundred = numbers_name[left / 100] + " hundred"
      tens = numbers_name[number - ((number / 100) * 100) - (number % 10)]
      singles = numbers_name[number % 10]
      if hundred == "zero hundred"
        hundred = ""
      end
      if tens == "zero"
        tens = ""
      end
      thousand + " " + hundred + " " + tens + " " + singles
    elsif number.to_s.length == 5
      if number >= 20000
        return "Sorry, I'm not able to translate this number"
      else
        get_thousand = number/1000
        left = number - (get_thousand*1000)
        thousand = numbers_name[get_thousand] + " thousand"
        hundred = numbers_name[left / 100] + " hundred"
        tens = numbers_name[number - ((number / 100) * 100) - (number % 10)]
        singles = numbers_name[number % 10]
        if hundred == "zero hundred"
          hundred = ""
        end
        if tens == "zero"
          tens = ""
        end
        thousand + " " + hundred + " " + tens + " " + singles
      end
    end
  end

end

puts NumberToName.numbers_to_words(7)
puts NumberToName.numbers_to_words(42)
puts NumberToName.numbers_to_words(601)
puts NumberToName.numbers_to_words(2001)
puts NumberToName.numbers_to_words(1999)
puts NumberToName.numbers_to_words(19999)
